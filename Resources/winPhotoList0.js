...
*/					
	...
	var appendArticle = function(_articleList) {
		...
		if (_articleList.length < articleCount + 1) {
			// 取得した記事をテーブルに追加
			if (_type == "follow") {
				photoListTableView.appendRow(getPhotoListArticleTableRow(_articleList));
			} else {
		...
		// 取得した記事が表示件数より1件多い場合、「続きを読む」ボタンを表示
			...
			// 取得した記事をテーブルに追加
			if (_type == "follow") {
				photoListTableView.appendRow(getPhotoListArticleTableRow(_articleList), {animated:true});
			} else {
